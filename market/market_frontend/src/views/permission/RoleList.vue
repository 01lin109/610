...
import { getRoleList, getRole, addRole, updateRole, deleteRole } from '@/api/permission'
import { ElMessage } from 'element-plus'
const loading = ref(false), tableData = ref([]), dialogVisible = ref(false), dialogTitle = ref('新增角色'), formRef = ref(null), isEdit = ref(false)
const form = reactive({ id: null, roleName: '', roleKey: '', roleSort: 0, status: 1 })
const rules = { roleName: [{ required: true }], roleKey: [{ required: true }] }
const fetchData = async () => { loading.value = true; try { const res = await getRoleList(); tableData.value = res.data.records || res.data || [] } catch {} loading.value = false }
const handleAdd = () => { isEdit.value = false; dialogTitle.value = '新增角色'; dialogVisible.value = true }
const handleEdit = async row => { isEdit.value = true; dialogTitle.value = '编辑角色'; Object.assign(form, row); dialogVisible.value = true }
const handleDel = async id => { try { await deleteRole(id); ElMessage.success('删除成功'); fetchData() } catch (e) { ElMessage.error('删除失败') } }
const resetForm = () => { formRef.value?.resetFields(); Object.assign(form, { id: null, roleName: '', roleKey: '', roleSort: 0, status: 1 }) }
const submitForm = async () => {
  const valid = await formRef.value.validate().catch(() => false); if (!valid) return
  try {
    isEdit.value ? await updateRole(form) : await addRole(form)
